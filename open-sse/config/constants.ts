...
// Timeout waiting for the first SSE chunk after headers are received (ms).
// Helps combo fallback move to the next model when upstream accepts the
// request but never starts sending stream data.
export const STREAM_FIRST_CHUNK_TIMEOUT_MS = parseInt(
  process.env.STREAM_FIRST_CHUNK_TIMEOUT_MS || "30000",
  10
);
